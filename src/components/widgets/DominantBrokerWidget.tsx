  ...
  return (
    <div className="rounded-3xl p-5 sm:p-6 border border-border/40 bg-gradient-to-b from-surface-1/90 via-surface-1/50 to-surface-2/40 shadow-xl space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="overflow-x-auto rounded-2xl border border-line-2 bg-card">
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-line-2 tabular-nums">
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-3 px-4 font-bold text-muted-foreground">Kode Broker</td>
                {PERIODS.map((p) => {
                  const b = buyerMap[p]
                  return (
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Nilai Beli</td>
                {PERIODS.map((p) => (
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Nilai Beli Bersih</td>
                {PERIODS.map((p) => {
                  const val = buyerMap[p]?.net_val
                  return (
                    <td key={p} className="py-2.5 px-3 text-center font-black text-emerald-600 dark:text-emerald-400">
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Total Nilai Pasar</td>
                {PERIODS.map((p) => (
                  <td key={p} className="py-2.5 px-3 text-center text-muted-foreground/80 font-medium">
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Harga Beli Rata-rata</td>
                {PERIODS.map((p) => {
              ...
              {/* Row 7: Dominasi % */}
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Dominasi (%)</td>
                {PERIODS.map((p) => {
                  const dom = buyerMap[p]?.dominance_pct
                  return (
                    <td key={p} className="py-2.5 px-3 text-center font-black text-amber-600 dark:text-amber-400">
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-black uppercase tracking-widest text-rose-600 dark:text-rose-400">
        ...
        <div className="overflow-x-auto rounded-2xl border border-line-2 bg-card">
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-line-2 tabular-nums">
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Nilai Jual</td>
                {PERIODS.map((p) => (
                  <td key={p} className="py-2.5 px-3 text-center text-foreground font-semibold">
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Nilai Jual Bersih</td>
                {PERIODS.map((p) => {
                  const val = sellerMap[p]?.net_val
                  return (
                    <td key={p} className="py-2.5 px-3 text-center font-black text-rose-600 dark:text-rose-400">
              ...
              <tr className="hover:bg-surface-2/30 transition-colors">
                <td className="py-2.5 px-4 text-muted-foreground">Harga Jual Rata-rata</td>
                {PERIODS.map((p) => {
      ...
      {b1m && (
        <div className="p-3.5 rounded-2xl bg-surface-2/70 border border-line-2 flex items-start gap-3 text-xs leading-relaxed text-muted-foreground">
          ...
          <div>
            ...
            Dalam periode 1 Bulan terakhir, broker <strong className="text-amber-600 dark:text-amber-400">{b1m.broker_code} ({b1m.broker_name || 'Bandar Utama'})</strong> memegang akumulasi beli terbesar senilai <strong className="text-foreground">{formatVal(b1m.gross_val)}</strong> dengan harga modal rata-rata <strong className="text-foreground">Rp {b1m.avg_price?.toFixed(2)}</strong> (
            {pnl != null ? (
              <span className={pnl >= 0 ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-rose-600 dark:text-rose-400 font-bold'}>
