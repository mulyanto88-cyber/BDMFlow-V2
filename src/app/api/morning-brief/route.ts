...
export async function GET() {
  try {
    const [pulse, topRadar, groupRotation, alertFeed, stealth] = await Promise.all([
      run(`
        SELECT trading_date::VARCHAR AS date, stock_count::BIGINT AS stock_count, (total_value/1e12)::DOUBLE AS total_value_triliun,
               (total_foreign_flow/1e9)::DOUBLE AS foreign_net_miliar, whale_count::BIGINT AS whale_count, anomaly_count::BIGINT AS anomaly_count,
               gainers::BIGINT AS gainers, losers::BIGINT AS losers, avg_change_pct::DOUBLE AS avg_change_pct
        FROM market.tb_market_summary
        ORDER BY trading_date DESC LIMIT 1
      `),
      run(`
        SELECT r.stock_code, r.sector, r.group_name, s.close::DOUBLE AS close,
               ROUND(r.change_percent::DOUBLE,2) AS change_percent, r.radar_score::INTEGER AS radar_score,
      ...
      run(`
        ...
        SELECT sa.Code AS stock_code, sa.Price::DOUBLE AS price,
