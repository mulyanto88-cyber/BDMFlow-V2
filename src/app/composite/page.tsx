...
import { BarChart3, Activity, ArrowRight, AlertTriangle, Loader2, Shield } from 'lucide-react'
...
const columns = [
  ...
  col.accessor('composite_tier', { header: 'Signal', cell: r => <TierBadge tier={r.getValue() as string} /> }),
...
export default function CompositePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/morning-brief').then(r => r.json()).catch(() => null),
      fetch('/api/radar?action=list&min_score=0&min_close=100&min_value=5000000000&limit=30').then(r => r.json()).catch(() => ({ data: [] })),
      fetch('/api/radar?action=signals').then(r => r.json()).catch(() => ({ data: [] })),
    ]).then(([brief, radar, signals]) => {
      setPulse(brief?.pulse ?? null)
      setComposite(radar?.data ?? [])
      setAlerts(signals?.data ?? [])
    ...
    <div className="sidebar-offset min-h-screen">
      <div className="max-w-[1600px] mx-auto px-4 py-6 space-y-6 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
          <div>
            <h1 className="text-3xl font-black tracking-tight bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent">BDMFlow V2 — Composite Command Center</h1>
            <p className="text-sm text-muted-foreground mt-1">Multi-source convergence scoring · {composite.length} stocks</p>
          </div>
        ...
        {pulse && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ...
            <MetricCard label="Foreign Flow" value={fmtM(pulse.foreign_miliar ?? 0)} trend={(pulse.foreign_miliar ?? 0) >= 0 ? 'up' : 'down'} />
            <MetricCard label="Breadth" value={pulse.breadth_score ?? '—'} sub={`${pulse.gainers ?? 0}G / ${pulse.losers ?? 0}L`} />
            ...
            <MetricCard label="Avg Change" value={fmtP(pulse.avg_change_pct ?? 0)} trend={(pulse.avg_change_pct ?? 0) >= 0 ? 'up' : 'down'} />
          </div>
        )}
          ...
          <div className="space-y-4">
            {composite.slice(0, 3).map((s: any, i: number) => (
              <Link key={s.stock_code} href={`/stock/${s.stock_code}`} className="glass rounded-xl p-4 border border-white/5 card-hover flex items-center gap-4 block">
                <ScoreGauge score={Number(s.radar_score ?? s.composite_score ?? 0)} size="sm" />
                <div>
                  ...
                  <p className="text-xs mt-0.5">{fmtP(s.change_percent ?? 0)} | 5D: {fmtP(s.return_5d ?? 0)}</p>
                </div>
              </Link>
            ))}

            <div className="glass rounded-xl p-4 border border-white/5">
              <h3 className="text-sm font-bold mb-3 flex items-center gap-2"><Activity size={14} className="text-red-400" /> Active Signals</h3>
              {alerts.length > 0 ? (
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {alerts.map((a: any) => (
                    <div key={a.composite_signal} className="flex items-center gap-2 text-xs p-2 rounded-lg hover:bg-white/[0.04]">
                      <span className="font-bold flex-1">{a.composite_signal}</span>
                      <span className="text-muted-foreground">{a.count} stocks</span>
                      <span className="font-black">{a.avg_score}</span>
                    </div>
                  ))}
                </div>
              ) : <p className="text-xs text-muted-foreground py-4 text-center">No active signals</p>}
            </div>
          </div>
        </div>
        ...
        <div className="glass rounded-xl p-4 border border-white/5 text-xs text-muted-foreground">
          ...
          <span className="text-gold-400 font-semibold">STRONG BUY ≥80</span> · <span className="text-emerald-400 font-semibold">BUY 65-79</span> · <span className="text-lime-400 font-semibold">ACCUMULATE 50-64</span>
        </div>
      </div>
    </div>
  )
